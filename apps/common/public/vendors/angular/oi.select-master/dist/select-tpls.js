...
.factory('oiUtils', ['$document', '$timeout', function($document, $timeout) {
    ...
    function getPositionInArray(elem, arr, filter, useOnlyWithElem) {
        var filteredElem = filter(elem);
        var currentElem;
        for (var i = 0; i < arr.length; ++i) {
            currentElem = !useOnlyWithElem ? filter(arr[i]) : arr[i];
            if (angular.equals(filteredElem, currentElem)) {
                return i;
            }
        }
    ...
    return {
        ...
        getOffset: getOffset,
        getPositionInArray: getPositionInArray
    }
}]);

angular.module('oi.select')

.directive('oiSelect', ['$document', '$q', '$timeout', '$interval', '$parse', '$interpolate', '$injector', '$filter', '$animate', 'oiUtils', 'oiSelect', function($document, $q, $timeout, $interval, $parse, $interpolate, $injector, $filter, $animate, oiUtils, oiSelect) {
    var NG_OPTIONS_REGEXP = /^\s*([\s\S]+?)(?:\s+as\s+([\s\S]+?))?(?:\s+group\s+by\s+([\s\S]+?))?(?:\s+disable\s+when\s+([\s\S]+?))?\s+for\s+(?:([\$\w][\$\w]*)|(?:\(\s*([\$\w][\$\w]*)\s*,\s*([\$\w][\$\w]*)\s*\)))\s+in\s+([\s\S]+?)(?:\s+track\s+by\s+([\s\S]+?))?$/;
    var VALUES_REGEXP     = /([^\(\)\s\|\s]*)\s*(\(.*\))?\s*(\|?\s*.+)?/;
    var SCROLL_TO_ACTIVE_INTERVAL = 30;
    var SCROLL_TO_ACTIVE_ATTEMPTS = 20;
    ...
    return {
        ...
        compile: function (element, attrs) {
            ...
            return function(scope, element, attrs, ctrl) {
	            ...
	            var loadMatchesDef = null;
	            var allowValuesNotInList = options.allowValuesNotInList;

                scope.isDragNDropActivated = false;
                scope.sortableConfig = {
                    disabled: !isReorderEnabled,
                    draggable: '.select-search-list-item_selection',
                    ghostClass: 'sortable-ghost',
                    onStart: function() {
                        scope.isDragNDropActivated = true;
                    },
                    onEnd: function() {
                        scope.isDragNDropActivated = false;
                    },
                    onUpdate: function() {
                        ...
                        if (multiple) {
                            var result = [];
                            angular.forEach(scope.output, function(value) {
                                result.push(selectAsFn ? selectAs(value) : value);
                            });
                ...
                if (isReorderEnabled) {
                    element.on('mousedown', '.select-search-list-item_selection', function() {
                       element.find('.tooltip').hide();
                    });
                }

                scope.autocompleteOnly = isAutocompleteOnly;
                scope.isNewItemUseLabel = isNewItemUseLabel;
                scope.newItemLabel = options.newItemLabel;
	            scope.isReorderEnabled = isReorderEnabled;

	            if (isAutocompleteOnly) {
		            element.find('.select-search').addClass('autocomplete-only');
	            }
                ...
                var multipleLimitWatchClear = scope.$parent.$watch(attrs.multipleLimit, function(value) {
	                multipleLimit = Number(value) || Infinity;
	                multipleLimitWatchClear();
                });

                var multipleWatchClear = scope.$parent.$watch(attrs.multiple, function(multipleValue) {
                    multiple = multipleValue === undefined ? angular.isDefined(attrs.multiple) : multipleValue;
                    multipleWithCheckboxes = multiple && !isAutocompleteOnly;
                    scope.isMultiple = multiple;
                    scope.isMultipleWithCheckboxes = multipleWithCheckboxes;
                    ...
                    if (selectAsFn && exists(value)) {
                        promise = getMatches(null, value)
                            .then(function(collection) {
                                var intersection = oiUtils.intersection(output, collection, null, selectAs);
                                if (allowValuesNotInList || (!intersection.length && isNewItemUseLabel)) {
                                    return output;
                                }
                ...
                scope.$watch('groups', function(groups) {
                    if (oiUtils.groupsIsEmpty(groups) && (!isNewItemUseLabel || !scope.query)) {
                        scope.isOpen = false;
                ...
                scope.$watch('isOpen', function(isOpen) {
                    ...
                    if (isOpen) {
                        var attemptsCount = 0;
                        if (scrollToActiveIntervalPromise) {
                            $interval.cancel(scrollToActiveIntervalPromise);
                        }

                        scrollToActiveIntervalPromise = $interval(function() {
                            if (listElement.get(0).offsetHeight > 0) {
                                scrollToActiveElement();
                                $interval.cancel(scrollToActiveIntervalPromise);
                                return;
                            }
                            if (attemptsCount > SCROLL_TO_ACTIVE_ATTEMPTS) {
                                $interval.cancel(scrollToActiveIntervalPromise);
                                return;
                            }
                            attemptsCount++;
                        }, SCROLL_TO_ACTIVE_INTERVAL);
                    }
                ...
                scope.toggleItem = function(option) {
                    if (scope.isSelected(option)) {
                        var position;
                        if (selectAsFn) {
                            position = oiUtils.getPositionInArray(option, ctrl.$modelValue, selectAs, true);
                        } else {
                            position = oiUtils.getPositionInArray(option, ctrl.$modelValue, trackBy);
                        }

                        if (position !== -1) {
                            scope.removeItem(position);
                        }
                    } else {
                        scope.addItem(option);
                    }
                };

                scope.isSelected = function(option) {
                    return (oiUtils.intersection(scope.output, [option], trackBy, trackBy).length > 0);
                };
                ...
                scope.selectAll = function() {
                    if (multiple) {
                        ...
                        angular.forEach(scope.groups, function(group) {
                            angular.forEach(group, function(option) {
                                result.push(option);
                            });
                        });
	                    if (multipleLimit) {
		                    result = result.slice(0, multipleLimit);
	                    }
                ...
                scope.addNewOption = function() {
                    if (newItemFn && scope.query) {
                        scope.showLoader = true;
                        $q.when(newItemFn(scope.$parent, {$query: scope.query})).then(function(data) {
                ...
                function click(event) {
                    ...
                    if (scope.isOpen && options.closeList && (event.target.nodeName !== 'INPUT' || !scope.query.length)) { //do not reset if you are editing the query
                        if (!multipleWithCheckboxes) {
                            resetMatches({
                                query: (options.editItem && !editItemIsCorrected) || (isNewItemUseLabel && $(event.target).closest('.new-item-label').length)
                            });
                        }
                        scope.$evalAsync();
                    } else {
                ...
                function saveOn(query, triggerName) {
                    ...
                    if (isTriggered && ((isNewItem && !isNewItemUseLabel) || selectedOrder && !getDisableWhen(selectedOrder))) {
                        scope.showLoader = true;
                ...
                function getMatches(query, selectedAs) {
                    ...
                    timeoutPromise = $timeout(function() {
	                    ...
	                    if (loadMatchesDef) {
		                    loadMatchesDef.reject();
	                    }

	                    var def = $q.defer();
	                    loadMatchesDef = def;

	                    def.promise.then(function(values) {
		                    ...
		                    if (values && !selectedAs) {
			                    ...
			                    var withoutIntersection = (!multipleWithCheckboxes) ? oiUtils.intersection(filteredList, outputValues, trackBy, trackBy, true) : filteredList;
...
angular.module("oi.select").run(["$templateCache", function($templateCache) {$templateCache.put("src/template.html","<div class=select-search><ul class=select-search-list><li><ul ng-if=::isReorderEnabled class=sortable-container ng-sortable=sortableConfig><li class=\"btn btn-default btn-xs select-search-list-item select-search-list-item_selection\" ng-hide=listItemHide ng-repeat=\"item in output track by $index\" ng-class=\"{focused: backspaceFocus && $last}\" bind-html-compile=getSearchLabel(item)></li></ul><ul ng-if=::!isReorderEnabled><li class=\"btn btn-default btn-xs select-search-list-item select-search-list-item_selection\" ng-hide=listItemHide ng-repeat=\"item in output track by $index\" ng-class=\"{focused: backspaceFocus && $last}\" bind-html-compile=getSearchLabel(item)></li></ul></li><li class=\"select-search-list-item select-search-list-item_input\" ng-class=\"{\'select-search-list-item_hide\': inputHide}\"><input autocomplete=off ng-model=query ng-keyup=keyUp($event) ng-keydown=keyDown($event)></li><li class=\"select-search-list-item select-search-list-item_loader\" ng-show=showLoader></li></ul></div><div class=\"select-dropdown hidden\" ng-class=\"{hidden: !isOpen}\" ng-show=isOpen><div ng-if=::isMultipleWithCheckboxes class=select-dropdown-multiple-control-labels><a class=select-all ng-click=selectAll()>Select All</a> <a class=select-none ng-click=selectNone()>Select None</a></div><div class=select-dropdown-optgroup-container><ul ng-if=isOpen class=select-dropdown-optgroup ng-repeat=\"(group, options) in groups\"><div class=select-dropdown-optgroup-header ng-if=\"group && options.length\" ng-bind-html=\"getGroupLabel(group, options)\"></div><li class=select-dropdown-optgroup-option ng-if=::!isMultipleWithCheckboxes ng-init=\"isDisabled = getDisableWhen(option)\" ng-repeat=\"option in options\" ng-class=\"{\'active\': selectorPosition === groupPos[group] + $index, \'disabled\': isDisabled, \'ungroup\': !group}\" ng-click=\"isDisabled || addItem(option)\" ng-mouseenter=\"setSelection(groupPos[group] + $index)\" ng-bind-html=getDropdownLabel(option)></li><li class=select-dropdown-optgroup-option ng-if=::isMultipleWithCheckboxes ng-init=\"isDisabled = getDisableWhen(option)\" ng-repeat=\"option in options\" ng-class=\"{\'active\': selectorPosition === groupPos[group] + $index, \'disabled\': isDisabled, \'ungroup\': !group}\" ng-click=\"getDisableWhen(option) || toggleItem(option)\" ng-mouseenter=\"setSelection(groupPos[group] + $index)\"><span class=\"multiple-value-selection checkbox\"><input type=checkbox ng-checked=isSelected(option) class=multiple-select-checkbox> <label ng-bind-html=getDropdownLabel(option)></label></span></li></ul></div><div ng-if=\"isNewItemUseLabel && query\" class=new-item-label ng-click=\"isDisabled || addNewOption()\"><a class=new-item-label-link>{{newItemLabel}}</a></div></div>");}]);