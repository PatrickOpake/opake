...
<div class="staff-pref-card-export">
	<div>
		<span class="title">PREFERENCE CARD</span>
		<?php if($card->template->name):?><span class="template-name"> - <?= $card->template->name; ?></span><?php endif; ?>
	</div>
	<div class="subtitle"><span class="bold-text">Surgeon:</span> <?= ((isset($user)) ? $user->getFullName() : ((isset($card)) ? $card->user->getFullName() : '')) ?> </div>
	<div class="subtitle">
	...
	<?php if (isset($stages_with_items)): ?>
		...
		<?php endforeach; ?>
	<?php endif ?>
	<div class="subtitle items-title"><span class="bold-text">Additional Notes:</span></div>
	<?php if (isset($card) && isset($card->additional_note)): ?>
		<div class="additional-notes">
			<?= $card->additional_note ?>
		</div>
	<?php endif ?>

	<div class="subtitle items-title"><span class="bold-text">Variable Cost:</span></div>
	<?php if (isset($card) && isset($card->var_cost)): ?>
		<div><?= $_money($card->var_cost) ?></div>
	<?php endif ?>
</div>
</body>
</html>