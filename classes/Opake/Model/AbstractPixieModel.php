 ...
 *
 * @method AbstractPixieModel limit(int $limit = null) Set number of rows to return .
 *               If NULL is passed than no limit is used.
 *               Without arguments returns current limit, returns self otherwise.
 *
 * @method AbstractPixieModel offset(int $offset = null) Set the offset for the first row in result.
 *               If NULL is passed than no limit is used.
 *               Without arguments returns current offset, returns self otherwise.
 *
 * @method AbstractPixieModel order_by(string $column, string $dir = null) Adds a column to ordering parameters
 *
 * @method AbstractPixieModel where(mixed $key, mixed $operator = null, mixed $val = null) behaves just like Query_Database::where()
 *
 * @property \PHPixie\DB\Query $query
 *
 * @property \PHPixie\DB\Connection $conn
 *
 */
class AbstractPixieModel
{
	...
	/**
	 * Flag whether relations were normalized or not
	 * @var bool
	 */
	protected $is_relations_normalized = false;

	/**
	 * Associated query builder
	 * @var \PHPixie\DB\Query
	 */
	protected $_current_query;
	...
	public function __construct($pixie)
	{
		...
		if ($this->table == null) {
			...
			$matches = [];
			if (preg_match('/^(.+)_model_(.+)$/', $tableName, $matches)) {
				if (isset($matches[1])) {
					$this->table = $this->plural($matches[1]);
	...
	public function __get($column)
	{
		...
		if ($target = $this->pixie->arr($relations, $column, false)) {
			...
			if ($target['type'] == 'has_many' && isset($target['through'])) {
				...
				if (!empty($target['overwrite']['ordering'])) {
					$orderedColumn = (isset($target['overwrite']['order_column'])) ? $target['overwrite']['order_column'] : 'order';
					$model->query->order_by($through_alias . '.' . $orderedColumn);
				}
			} else {
				$last_alias = $model->query->last_alias();
				$new_alias = $model->query->add_alias();
	...
	public function __set($column, $val)
	{
		...
		if ($column === 'conn') {
			$this->set_current_connection($val);
			return;
		}

		if ($column === 'query') {
			$this->set_current_query($val);
			return;
		}
	...
	*/
	...
	protected function set_current_query($query)
	{
		$this->_current_query = $query;
	}

	protected function set_current_connection($conn)
	{
		$this->_current_connection = $conn;
	}

	protected function get_current_query()
	{
		if (!$this->_current_query) {
			$this->_current_query = $this->conn->query('select');
			$this->_current_query->table($this->table);
		}

		return $this->_current_query;
	}

	protected function get_current_connection()
	{
		if (!$this->_current_connection) {
			$this->_current_connection = $this->pixie->db->get($this->connection);
		}

		return $this->_current_connection;
	}

	protected function normalize_relations()
	{
		if (!$this->is_relations_normalized) {
			...
			foreach (array('belongs_to', 'has_one', 'has_many') as $rels) {
				...
				foreach ($this->$rels as $key => $rel) {
					...
					if (!isset($rel['key'])) {
						$normalized[$key]['key'] = $this->model_key($rels != 'belongs_to' ? $this->table : $rel['model']);
					}
			...
			$this->is_relations_normalized = true;
		}
	}
