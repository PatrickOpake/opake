...
class PreOperativeForm extends AbstractForm
{
	...
	{
		$validator->field('height_ft')->rule('numeric', $this)->error('Height must be numeric');
		$validator->field('height_in')->rule('numeric', $this)->error('Height must be numeric');
		$validator->field('weight_lbs')->rule('numeric', $this)->error('Weight must be numeric');
		...
		$validator->field('primary_care_name')->rule('max_length', static::CHARACTER_LIMIT)
			->error(static::CHARACTER_LIMIT . '  character limited exceeded in Primary Care Name');
		$validator->field('transportation_name')->rule('max_length', static::CHARACTER_LIMIT)
			->error(static::CHARACTER_LIMIT . '  character limited exceeded in Transportation Name');
		$validator->field('caretaker_name')->rule('max_length', static::CHARACTER_LIMIT)
			->error(static::CHARACTER_LIMIT . '  character limited exceeded in Caretaker Name');

		$validator->field('conditions')->rule('callback',  [$this, 'checkConditionsLength'])
