...
class BenefitResponseSubscriber extends Loop
{
	...
	public function __construct($s)
	{
		...
		$builder = '';
		while ($stringQueue->hasNext() && !StringHelper::startsWith($stringQueue->peekNext(), 'HL')) {
			$builder .= $stringQueue->getNext();
		}

		$splitArray = SegmentString::split($builder, "EB");
		...
		$dArray = SegmentString::split($builder, "HL");
		foreach ($dArray as $str) {
			$this->dependents[] = new BenefitResponseDependent($str);
		}
	}
	...
	function loadDefinition()
	{
		...
		foreach ($this->dependents as $item) {
			$this->messagesDefinition[] = $item;
		}
	}
