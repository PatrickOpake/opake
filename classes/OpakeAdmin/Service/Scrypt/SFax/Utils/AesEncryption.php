...
namespace OpakeAdmin\Service\Scrypt\SFax\Utils;
...
class AesEncryption
{
	...
	{
		return call_user_func_array(array('\OpakeAdmin\Service\Scrypt\SFax\Utils\PadCrypt', 'pad_' . $this->allowed_paddings[$this->padding]), array($text, $block_size));
	}

	private function unpad($text)
	{
		return call_user_func_array(array('\OpakeAdmin\Service\Scrypt\SFax\Utils\PadCrypt', 'unpad_' . $this->allowed_paddings[$this->padding]), array($text));
	}
